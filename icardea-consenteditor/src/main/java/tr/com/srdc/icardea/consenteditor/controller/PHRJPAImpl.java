...
public class PHRJPAImpl implements PHRInterface {
	...
	public List<ConsentDocument> getConsentDocuments(String patientId) {
	    ...
	    Query query = em.createQuery("select cd from ConsentDocument cd where cd.consentDocumentMetaData.patientId ='" + patientId + "'");
	...
	public ConsentDocument getCurrentConsentDocumentByPatientId(String patientID) {
		logger.info("*************************");
		EntityManager em = emf.createEntityManager();
		String queryString = "select cd from ConsentDocument cd where cd.isCurrent='1' and cd.consentDocumentMetaData.patientId = '" + patientID + "'";
		logger.info("QueryString2:" + queryString);
	...
	public void setCurrentConsentDocumentofPatient(ConsentDocument consentDocument) {
		EntityManager em = emf.createEntityManager();
	    Query query = em.createQuery("select cd from ConsentDocument cd where cd.consentDocumentMetaData.patientId = '" + consentDocument.getConsentDocumentMetaData().getPatientId() + "'");
	    List<ConsentDocument> cdList = query.getResultList();
	    em.getTransaction().begin();
	    for (int i = 0; i < cdList.size(); i++) {				
