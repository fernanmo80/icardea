...
public abstract class AbstractPDFParser {//implements PDFParser {
	 ...
	 * Logger object
	 */
	private static Logger logger = Logger.getLogger(AbstractPDFParser.class);
	
	/**
	 * Properties object for each portal implementation.
	 */
	protected Properties pdfParserProperties = new Properties();
	 ...
	 */
	public AbstractPDFParser(String propertyBaseFilename, String languageCode) throws InvalidParameterValueException, PropertyFileNotFoundException {
		...
		try {
			InputStream inputStream = AbstractPDFParser.class.getClassLoader().getResourceAsStream("config/pdfparser/" + propertyFilename);
			
			if (inputStream != null) {
				pdfParserProperties.load(inputStream);
			} else {
				throw new PropertyFileNotFoundException("Error reading pdf parsers property file: '" +
						propertyFilename + "'.");
			}
	...
	public String getProperty(String key) throws PropertyNotFoundException {
		String value = pdfParserProperties.getProperty(key);
		if (value == null) {
			throw new PropertyNotFoundException(
					"Property key '" + key + "' not found. Please " +
					"make sure that this key exists in the pdf parser" +
					"property file.");
		} // end if
		return value.trim();
	}
