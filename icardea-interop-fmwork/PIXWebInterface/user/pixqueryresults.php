					...
					<legend><?php echo $_RESULTS;?></legend><br><BR>
						<?php
								if(isset($_POST['msg'])){
									...
									//$res = GetTCPResponse1('139.91.190.45','2575',$_POST['msg']) ;
									$res = GetTCPResponse1($_POST['msg']) ;
									//echo $_POST['msg'];echo '<br/>';
									//echo 'socket_response = '.$res;
									//exit;
									...
									if($err_seg=='' && $msa_res=='AA' && $qak_res=='OK'){
										...
										for($i=0;$i<count($pid_seg);$i++){
											...
											if(count($results) >= 3) {
												...
												if ($has_epsos_id == 1){
													$pid_array=explode("^", $results[3]);
													$epsos_id=$pid_array[0];
													$auth_array=explode("&", $pid_array[3]);
													$enamespace_id=$auth_array[0];
													$euniversal_id=$auth_array[1];
													$euniversal_id_type=$auth_array[2];
												}
										?>
									...
									<?php
										...
										echo $err_seg . '<br>ERROR OCCURRED';
								}
							?>
					</fieldset>
					<!-- ============================== Fieldset 1 end ============================== -->
