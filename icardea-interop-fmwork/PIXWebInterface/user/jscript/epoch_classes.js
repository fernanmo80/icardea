...
function newCalendar(elem,date1)
{
	if (cal != null) 
		cal.hide(); 
		
	cal = new Epoch('cal','popup',elem,false,date1);
	cal.show();
}

//constructor for the main Epoch class (ENGLISH VERSION)
function Epoch(name,mode,targetelement,multiselect,date1)
{
	this.state = 0;
	this.name = name;
	if (date1=='')
	...
	{
		this.curDate=new Date(date1.substring(0,4),(date1.substring(4,6))-1);
	}
	this.mode = mode;
	...
	//this.selectedData = new Date(2000,00,10);
...
{
	//this.daylist = new Array('Κυ','Δε','Τρ','Τε','Πε','Πα','Σα','Κυ','Δε','Τρ','Τε','Πε','Πα','Σα'); /*<lang:en>*/
	//this.months_sh = new Array('Ιαν','Φεβ','Μαρ','Απρ','Μαι','Ιον','Ιολ','Αυγ','Σεπ','Οκτ','Νοε','Δεκ');
	...
	//this.clearbtn_caption = 'Καθαρισμός';
	...
	//this.clearbtn_title = 'Καθαρίζει τις επιλεγμένες ημερομηνίες στο ημερολόγιο';
	//this.maxrange_caption = 'Αυτή είναι η μέγιστη ημερομηνία';
	
	this.closebtn_caption = 'close';
	this.closebtn_title = 'close';
};
//-----------------------------------------------------------------------------
...
Epoch.prototype.createMainHeading = function () //PRIVATE: Creates the primary calendar heading, with months & years
{
	...
	monthUp.setAttribute('className','button');
	monthUp.setAttribute('title',this.monthup_title);
	monthDn.setAttribute('type','button');
	monthDn.setAttribute('value','<');
	monthDn.setAttribute('className','button');
...
Epoch.prototype.createFooter = function () //PRIVATE: creates the footer of the calendar - goes under the calendar cells
{
	...
	clearSelected.setAttribute('className','button');
	clearSelected.owner = this;
	clearSelected.onclick = function() { this.owner.resetSelections(false);};
	container.appendChild(clearSelected);
	var closeCal = document.createElement('input');
	closeCal.setAttribute('id','close_cal_btn');
	closeCal.setAttribute('type','button');
	closeCal.setAttribute('value',this.closebtn_caption);
	closeCal.setAttribute('title',this.closebtn_title);
	closeCal.setAttribute('className','button');
	closeCal.owner = this;
	closeCal.onclick = function() { this.owner.hide();};
	...
	if(!owner.selCurMonthOnly || cell.date.getMonth() == owner.displayMonth && cell.date.getFullYear() == owner.displayYear)
	{
		...
		else //if we can only select one cell at a time
		{
			...
			if(owner.tgt) //if there is a target element to place the value in, do so
			{
				...
				//owner.tgt.value = theDate.getFullYear() + "-" + (theDate.getMonth()+1) + "-" + theDate.getDate();// + " "+ getCurrentTime();
				owner.tgt.value =  (theDate.getFullYear()).toString()+((((theDate.getMonth()+1) < 10) ? '0' : '') + (theDate.getMonth()+1)).toString()+(((theDate.getDate() < 10) ? '0' : '') + theDate.getDate()).toString() ;
				//owner.tgt.value =  (theDate.getFullYear()).toString()+((this.addZero(theDate.getMonth()+1))).toString()+ (this.addZero(theDate.getDate())).toString();
...
function getCurrentTime()
{
	...
	return nhours+":"+nmins+":"+nsecn+" "+AorP;
} 
//-----------------------------------------------------------------------------
