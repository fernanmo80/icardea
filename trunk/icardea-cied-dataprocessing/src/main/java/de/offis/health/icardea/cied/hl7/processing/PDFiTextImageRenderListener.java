 ...
 *
 */
public class PDFiTextImageRenderListener implements RenderListener {
	/**
	 * Logger object
	 */
	@SuppressWarnings("unused")
	private static Logger logger = Logger.getLogger(PDFiTextImageRenderListener.class);
	 ...
	 */
	@SuppressWarnings("unused")
	private PDFiTextImageRenderListener() {}
	 ...
	 * @throws Exception
	 */
	public PDFiTextImageRenderListener(String baseExportDirectoryPath, String baseFileNameWithoutExtension) throws Exception {
		...
		logger.info("baseExportDirectoryPath" +baseExportDirectoryPath);
	...
	public void renderImage(ImageRenderInfo renderInfo) {
		try {
			...
			String fullExportFileNameWithoutExtension = this.baseExportDirectoryPath 
			+ this.baseFileNameWithoutExtension 
			+ "_(" + (getPageNumber()>0?"p"+pageNumberFormat.format(getPageNumber()):"p"+pageNumberFormat.format(0)) 
			+ "_ref" + refNumberFormat.format(renderInfo.getRef().getNumber()) 
			...
			if (PdfName.DCTDECODE.equals(filter)) {
				...
				if (awtimage != null) {
					logger.info("Image Height "+awtimage.getHeight()+ " & "+"Image Width "+awtimage.getWidth());
					if (awtimage.getHeight()==59 && awtimage.getWidth()==294){
						//Height 59, Width 294 is the Medtronic Logo
					}
					else{
					ImageIO.write(awtimage, "png", new FileOutputStream(fullExportFileNameWithoutExtension + ".png"));
					logger.info("fullExportFileNameWithoutExtension "+fullExportFileNameWithoutExtension+ ".png");
					}
				} // end if
			} // end if..else
