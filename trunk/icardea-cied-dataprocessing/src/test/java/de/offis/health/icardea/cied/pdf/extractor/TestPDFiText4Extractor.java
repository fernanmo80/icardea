...
/**
 * <p>
 * This class will test the iText v4.2.0 based PDF extractor class {@link PDFiText4Extractor}.
 * </p>
 * 
 *
 */
public class TestPDFiText4Extractor {
	/**
	 * Logger object
	 */
	private static Logger logger = Logger.getLogger(TestPDFiText4Extractor.class);
	...
	public void setUp() throws Exception {
		...
		if (pdfFileName == null) {
			...
			this.medtronicExtrFilePath = GlobalTools.checkDirectoryPath(medtronicPDFTestFileDirectory.getAbsolutePath()) + "extr_iText4";
			
			this.sjmExampleFiles = sjmPDFTestFileDirectory.listFiles(new PDFFilenameFilter());
			this.sjmExtrFilePath = GlobalTools.checkDirectoryPath(sjmPDFTestFileDirectory.getAbsolutePath()) + "extr_iText4";
		} else {
	...
	public void openPDFFilesSuccess() throws IOException, Exception {
		...
		try {
			...
			for (int i = 0; i < this.medtronicExampleFiles.length; i++) {
				pdfExtractor = new PDFiText4Extractor();
