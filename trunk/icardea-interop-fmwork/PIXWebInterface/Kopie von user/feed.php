	...
	<?php
	 ...
	 <div id="maincontent">
          ...
          <form action="feed.php" method="post">
				...
				<?php
					...
					<table class="main22">
						<tr>
							<td align=center>
								<input class="submit" type="submit" alt="SUBMIT" name="Submit" value="<?php if ($_POST['adt_type']=='A08'){echo $_UPDATE_SUBMIT;}else{echo $_SUBMIT2;} ?>" onClick="return CheckFields();" /></p>
		...
		<?php if(!isset($_POST['update']) && isset($_POST['protocol_id']) && isset($_POST['implant_id']) &&  isset($_POST['firstname']) && isset($_POST['lastname']) && isset($_POST['sex']) && isset($_POST['his_id1']) ){
					...
					$pid='PID|||'.trim($_POST['protocol_id']).'^^^icardea.pix&1.2.826.0.1.3680043.2.44.248240.1&ISO~'.trim($_POST['his_id1']).'^^^ORBIS&www.salk.at&DNS~'.trim($_POST['implant_id']).'^^^CIED&bbe3a050-079a-11e0-81e0-0800200c9a66&UUID||'.trim($_POST['lastname']).'^'.trim($_POST['firstname']).'||'.trim($_POST['birthdate']).'|'.trim($_POST['sex']).'|||'.trim($_POST['address']).'^^'.trim($_POST['city']).'^'.trim($_POST['state']).'^'.trim($_POST['postal']).'||||||||'.chr(13);
					$pv1='PV1||O'.chr(13);
					$hl7=$msh.$evn.$pid.$pv1;
					//echo $hl7;
					//echo '<br>';
					$res=GetTCPResponse($hl7);
