package tr.com.srdc.icardea.gui.dividedbox{
	//import flash.geom.*;
	...
	[Event(name="buttonClick", type="tr.com.srdc.icardea.gui.dividedbox.ButtonClickEvent")]
	public class ExtendedDividedBox extends DividedBox
	{
		...
		[Embed(source="/assets/dividedbox/Arrow_Down.png")]
		private var Arrow_Down:Class;
		
		[Embed(source="/assets/dividedbox/Arrow_Up.png")]
		private var Arrow_Up:Class;
				
		[Embed(source="/assets/dividedbox/Arrow_Close.png")]
		private var Arrow_Close:Class;
		
		[Embed(source="/assets/dividedbox/Arrow_Open.png")]
		...
		private function scrollHandler(event:ScrollEvent):void{
			_button.y = stage.stageHeight / 2 + event.position - 50;
		}
		...
		override protected function updateDisplayList(unscaledWidth:Number, unscaledHeight:Number):void{
			...
			if(_button){
				if(_flag){
					if(stage){
						_button.y = stage.stageHeight / 2 - 50;
						_difference = this.height - stage.stageHeight;
						_initialY = _button.y;
						//flag = false;
					}
				}
		...
		private function Draw_Gradient_Fill():void{
			...
			for(var i:int=0;i < mBoxDivider.length;i++) {
				...
				if (!_barFillColors){
					...
					if (!_barFillColors){
						_barFillColors =[0x999999,0x999999]; // if no style default to orange
					}
				}
				...
				if (!_barBorderColor){
					...
					if (!_barBorderColor){
						_barBorderColor =0x999999 ; // if no style default to orange
					}
				}
